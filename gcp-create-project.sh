...
gcloud compute networks subnets create --range 192.168.99.0/24 --network px-net px-subnet --region $GCP_REGION --project $GCP_PROJECT
...
gcloud compute firewall-rules create allow-px-tcp --allow=tcp:9001-9022 --network px-net --project $GCP_PROJECT
gcloud compute firewall-rules create allow-px-udp --allow=udp:9002 --network px-net --project $GCP_PROJECT
